...
import { History, TrendingUp, Calendar, Zap, Weight, ChevronDown, Activity } from 'lucide-react';
                ...
                <CardContent className="pb-6">
                  <div className="grid grid-cols-2 gap-4 border-t border-border/40 pt-6">
                      <div className="flex flex-col">
                          ...
                          <span className="flex items-center gap-1 text-xl font-black text-accent-amber italic">
                              ...
                              {session.totalSets} <span className="ml-1 text-[10px] uppercase opacity-40">Sets</span>
                          </span>
                      </div>
                      <div className="flex flex-col">
                  ...
                  <CollapsibleContent className="mt-6 animate-in space-y-4 duration-300 slide-in-from-top-2">
                    ...
                    <div className="space-y-3">
                       ...
                       {Object.entries(session.logs).map(([slotIdx, slotLogs]) => {
                          const muscleName = slotLogs[0]?.muscleName || `Group ${Number(slotIdx) + 1}`;
                          return (
                            <div key={slotIdx} className="space-y-3 rounded-xl border border-border/10 bg-background/20 p-4">
                               <div className="flex items-center justify-between border-b border-border/10 pb-2">
                                  ...
                                  <span className="text-[8px] font-bold uppercase opacity-40">{slotLogs.length} sets</span>
                               </div>
                               ...
                               {slotLogs.reduce((acc, log) => {
                                   ...
                                   <div key={gIdx} className="space-y-1.5">
                                      <div className="text-[10px] font-black uppercase italic opacity-80">{group.machineName}</div>
